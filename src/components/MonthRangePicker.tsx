...
function MiniCalendar({
  ...
  return (
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400 mb-1.5">{titolo}</p>
      <div className="flex items-center justify-between mb-2">
        <button
          type="button"
          onClick={() => onYearChange(year - 1)}
          className="w-6 h-6 rounded-md hover:bg-gray-100 text-gray-500 flex items-center justify-center"
          ...
          return (
            <button
              ...
              className={`text-xs rounded-lg py-1.5 transition-colors ${
                isSelected ? "bg-brand text-white font-semibold" : "text-gray-700 hover:bg-gray-100"
              }`}
            >
              {nome}
            </button>
...
export function MonthRangePicker({ da, a, onChange }: Props) {
  ...
  return (
    <div className="relative" ref={rootRef}>
      ...
      {open && (
        <div className="absolute z-20 mt-2 w-[420px] rounded-2xl border border-gray-200 bg-white shadow-lg p-4 flex gap-4">
          <div className="w-32 flex-shrink-0 border-r border-gray-100 pr-3 space-y-0.5">
            {PRESET.map((p) => (
              <button
                key={p.label}
                type="button"
                ...
                className="w-full text-left text-xs px-2 py-1.5 rounded-lg text-gray-700 hover:bg-gray-100 transition-colors"
              >
                {p.label}
              </button>
            ))}
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
              ...
              <button
                type="button"
                onClick={applica}
                className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-cta hover:bg-cta-dark text-white transition-colors"
              >
                Applica
              </button>
            </div>
