...
describe("riepilogoAppuntamenti", () => {
  it("nessun appuntamento -> tutti zero", () => {
    expect(riepilogoAppuntamenti([])).toEqual({ totali: 0, confermati: 0, annullati: 0 });
  });

  it("conta confermati e annullati separatamente dal totale", () => {
    const lista = [
    ...
    expect(riepilogoAppuntamenti(lista)).toEqual({ totali: 3, confermati: 2, annullati: 1 });
  });
  ...
  it("esclude gli appuntamenti eliminati dal totale", () => {
    ...
    expect(riepilogoAppuntamenti(lista)).toEqual({ totali: 1, confermati: 1, annullati: 0 });
  });
  ...
  it("uno stato diverso da confirmed/cancelled (es. non ancora osservato in produzione) non è né confermato né annullato", () => {
    ...
    expect(riepilogoAppuntamenti(lista)).toEqual({ totali: 1, confermati: 0, annullati: 0 });
  });
});
