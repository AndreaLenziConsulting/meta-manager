...
**Accesso automatico del consulente alla cartella Drive** (vedi `src/lib/driveAccesso.ts`) — ogni volta che `drive_folder_url` (colonna R) e `consulente_id` (colonna F) risultano entrambi impostati su un cliente (alla creazione o a una modifica successiva da `POST`/`PATCH /api/clienti`), l'app condivide in automatico quella cartella come collaboratore (ruolo Drive "writer") con l'email del consulente assegnato (tab Consulenti, colonna E) — con la notifica email standard di Google Drive. Idempotente: un consulente già collaboratore non viene ri-condiviso né ri-notificato; un consulente sostituito NON perde l'accesso da solo (va rimosso a mano su Drive se necessario). Mai bloccante: un fallimento (tipicamente la cartella non condivisa in scrittura con l'account del team) non fa fallire la creazione/modifica del cliente. `POST /api/admin/condividi-cartelle-clienti` (solo admin) applica la stessa logica in un colpo a tutti i clienti già esistenti — usata una tantum per i clienti collegati prima dell'introduzione di questa funzionalità, sicura da rilanciare.
...
**Prospect** — anagrafica persistente di un potenziale cliente. Un prospect si crea dalla UI (`+ Nuovo prospect`, tab Prospect del commerciale); colonne B→F si aggiornano anche da sole a ogni salvataggio di un report (vedi tab ReportCommerciale, non dettagliata qui). L'hand-off commerciale→consulente è in due passi: il commerciale "segnala come vinto" e suggerisce (facoltativo) un consulente in colonna S (`PATCH /api/prospect`); solo l'admin esegue davvero "Converti in cliente" (`POST /api/prospect/converti`), che crea il Cliente + una prima Sede e scrive il `cliente_id` risultante in colonna R — **senza** disattivare il prospect (colonna H), altrimenti sparirebbe da ogni lista prima ancora di poterne vedere l'esito.
| A prospect_id | B ragione_sociale | C tipo_business | D fatturato | E sedi | F email | G commerciale_id | H attivo | I creato_il | J drive_folder_url | K media_budget_mensile | L target_cpl | M target_cpa_appuntamento | N target_lead_settimana | O target_appuntamenti_settimana | P target_fatturato_mensile | Q target_margine_vendita_pct | R cliente_id | S consulente_suggerito_id |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
